...
\\
The use of a support shape for interactions can been seen as an additional filtering step that reduces the set of candidate points to only those that follow the curvature of the shape. Section \ref{sec:pointFiltering} gives a brief overview on this filtering step which is used throughout all assisted interactions. 
\\
...
\begin{figure}
    ...
    \caption[Screenshot of Shape-assisted Point Picking]
		{\textit{This figure shows the basic workflow for the assisted interactions. First, a shape is picked, from this picked shape larger cluster is created. }
