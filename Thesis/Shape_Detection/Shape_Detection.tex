...
Shape detection is an automated approach to detect geometric primitives in point clouds. Results are usually computed offline, as the computation takes an extensive amount of time. This thesis utilizes shape detection in a semi-automatic way. Rather than performing shape detection on the complete point cloud, the procedure is performed only on a small subset of points at a time. Larger coherent shapes are later reconstructed by clustering neighboring shapes that share similarities. 
